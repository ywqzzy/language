...
class Parser:
    ...
    def if_expr_c(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'ELSE'): #TODO
            res.register_advancement()
            self.advance()
            ...
            if self.current_tok.type == TT_NEWLINE:
                ...
                if self.current_tok.matches(TT_KEYWORD, 'END'): #TODO
                    res.register_advancement()
                    self.advance()
                else:
                    return res.failure(InvalidSyntaxError(
    ...
    def if_expr_b_or_c(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'ELIF'): #TODO
    ...
    def if_expr_cases(self, case_keyword):
        ...
        if not (self.current_tok.matches(TT_KEYWORD, case_keyword)):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected {case_keyword}'" #TODO add chinese support
            ))
        res.register_advancement()
        self.advance()
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            statements = res.register(self.statements())
            if res.error: return res
            cases.append((condition, statements, True)) #PAY ATTENTION TO IT

            if self.current_tok.matches(TT_KEYWORD, 'END') or self.current_tok.matches(TT_KEYWORD, '终结'): #TODO
                res.register_advancement()
                self.advance()
            else:
    ...
    def for_expr(self):
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            if not self.current_tok.matches(TT_KEYWORD, 'END'): #TODO
                return res.failure(InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end,
                    f"Expected 'END'"
                ))
